...
class DNSNewPage(Navigable):  # pylint: disable=too-many-instance-attributes
    ...
    def create_form(self, dns_policy: DNSPolicy):
        ...
        namespace = dns_policy.model.metadata.namespace
    ...
    def create_yaml(self, dns_policy: DNSPolicy):
        ...
        self.page.wait_for_selector("text=DNSPolicy has been successfully enforced", timeout=60000)
    ...
    def page_displayed(self):
        """Check if the DNSPolicy creation page is displayed"""
        # Page can be in either form or YAML view, check for form fields
        try:
            self.policy_name.wait_for(state="visible", timeout=10000)
            return True
        except PlaywrightTimeoutError:
            # If form not visible, check if YAML editor is visible
            try:
                self.editor.wait_for(state="visible", timeout=10000)
                return True
            except PlaywrightTimeoutError:
                return False
