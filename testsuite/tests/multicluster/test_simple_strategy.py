...
def test_simple_strategy(client, hostname, gateway, gateway2):
    ...
    a_record_set = set(map(lambda x: x.address, list(dns.resolver.resolve(hostname.hostname))))
    assert {gw1_ip, gw2_ip} == a_record_set, "Simple routing strategy should return both IP addresses in A record set"
