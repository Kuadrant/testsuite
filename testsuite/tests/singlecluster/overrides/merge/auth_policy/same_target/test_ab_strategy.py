...
def commit(request, route, authorization, override_merge_authorization):  # pylint: disable=unused-argument
    ...
    for policy in [authorization, override_merge_authorization]:  # Forcing order of creation.
        request.addfinalizer(policy.delete)
        policy.commit()
        policy.wait_for_accepted()


def test_multiple_policies_merge_default_ab(client, authorization, override_merge_authorization, user_auth, admin_auth):
