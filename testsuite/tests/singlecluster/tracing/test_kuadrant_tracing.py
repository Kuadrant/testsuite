...
def services(gateway):
    """List of expected service names that should appear in distributed traces."""
    return ["wasm-shim", "authorino", "limitador", f"{gateway.model.metadata.labels['app']}.kuadrant"]
...
def test_trace_includes_all_kuadrant_services(client, auth, tracing, services):
    """
    ...
    request_id = response.headers.get("x-kuadrant-request-id")
...
def test_send_reply_span_on_request_rejection(client, auth, tracing, expected_status_code, scenario):
    ...
    if scenario == "rate_limit":
        responses = client.get_many(
            "/get", 3, auth=auth, headers={"Traceparent": f"00-{os.urandom(16).hex()}-{os.urandom(8).hex()}-01"}
        )
        ...
        response = client.get("/get", headers={"Traceparent": f"00-{os.urandom(16).hex()}-{os.urandom(8).hex()}-01"})
