...
def policy_validate_span(policy_lifecycle_trace):
    ...
    if not spans:
        pytest.skip(f"No {policy_kind}.validate span found")
...
def policy_status_span(policy_lifecycle_trace):
    ...
    if not spans:
        pytest.skip(f"No {policy_kind} status span found")
...
def controller_reconcile_span(policy_lifecycle_trace):
    """Controller reconcile root span."""
    trace = policy_lifecycle_trace["trace"]

    spans = trace.filter_spans(lambda s: s.operation_name == "controller.reconcile")
    if not spans:
        pytest.skip("No controller.reconcile span found")
...
def test_policy_validation_before_status_update(policy_validate_span, policy_status_span):
    ...
    assert policy_validate_span.start_time < policy_status_span.start_time, \
...
def test_policy_spans_are_in_workflow_hierarchy(policy_lifecycle_trace, policy_validate_span, policy_status_span):
    ...
    # Validation span should be descendant of workflow (via validation parent)
    validation_parent = trace.get_span_by_id(policy_validate_span.get_parent_id())
    assert validation_parent is not None
    assert validation_parent.operation_name == "validation"

    validation_grandparent = trace.get_span_by_id(validation_parent.get_parent_id())
    assert validation_grandparent is not None
    assert validation_grandparent.operation_name == "workflow.data_plane_policies"

    # Status span should be descendant of workflow (via status_update parent)
...
def test_controller_reconcile_includes_event_details(policy_lifecycle_trace, controller_reconcile_span):
    ...
    assert f"{policy_kind}.kuadrant.io" in event_kinds, \
        f"event_kinds should contain {policy_kind}.kuadrant.io"
...
@pytest.mark.parametrize(
    ...
    [
        ("authorization", ["reconciler.auth_configs", "reconciler.istio_auth_cluster", "reconciler.authorino_istio_integration"]),
...
def test_policy_specific_reconcilers_executed(reconciler_spans, expected_reconcilers):
    ...
    for expected_reconciler in expected_reconcilers:
        assert expected_reconciler in reconciler_spans, \
            f"Should have {expected_reconciler} reconciler span"
...
@pytest.mark.parametrize("policy_lifecycle_trace", ["authorization", "rate_limit"], indirect=True)
def test_effective_policies_computed_before_reconcilers(policy_lifecycle_trace, effective_policies_span, reconciler_spans):
    """Validate effective policies are computed before reconcilers run."""
    trace = policy_lifecycle_trace["trace"]

    # Find effective_policies.compute child
    compute_spans = [
        s for s in trace.get_children(effective_policies_span.span_id)
    ...
    for reconciler_span in reconciler_spans.values():
        assert compute_span.start_time + compute_span.duration <= reconciler_span.start_time, \
