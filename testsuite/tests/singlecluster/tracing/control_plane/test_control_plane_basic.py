...
def test_policy_lifecycle_has_consistent_metadata(policy_lifecycle_trace):
    ...
    status_span = trace.filter_spans(
        lambda s: s.operation_name == f"policy.{policy_kind}"
        and s.get_tag("policy.name") == policy.name()
        and s.has_log_field("event", "policy status updated successfully")
    )[0]
...
def test_policy_spans_are_in_workflow_hierarchy(policy_lifecycle_trace):
    ...
    validation_parent = trace.get_span_by_id(validate_span.get_parent_id())
    ...
    status_parent = trace.get_span_by_id(status_span.get_parent_id())
