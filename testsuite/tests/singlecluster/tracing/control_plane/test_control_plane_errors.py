...
def orphan_test_route(request, cluster, blame, gateway, module_label, backend):
    ...
    orphan_route.commit()
    # No finalizer - test controls deletion
    return orphan_route
...
def test_orphaned_policy_reconciliation_traced(orphan_test_route, orphan_test_policy, tracing):
    ...
    policy_traces = tracing.get_traces(
        ...
        tags={
            "policy.name": orphan_test_policy.name(),
            "error": "true"
        }
    )
    ...
    for trace in policy_traces:
        error_spans.extend(
            trace.filter_spans(lambda s: s.has_log_field("exception.type", EXPECTED_ORPHAN_EXCEPTION_TYPE) and
                                         s.has_log_field("event", "exception") and
                                         s.has_log_field("exception.message", f"AuthPolicy target {orphan_test_route.name()} was not found")
                               )
        )
