...
def test_update_tls_policy_target_ref(
    ...
    response = KuadrantClient(base_url=f"https://{hostname2.hostname}", verify=True).get("/get")
    ...
    response = KuadrantClient(base_url=f"https://{hostname.hostname}", verify=True).get("/get")
    assert response.has_cert_verify_error()
