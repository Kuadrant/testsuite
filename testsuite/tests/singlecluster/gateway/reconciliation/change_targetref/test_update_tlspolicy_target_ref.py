...
def test_update_tls_policy_target_ref(
    ...
    assert tls_secret is not None, "TLS secret was not found after targetRef change"
    secret_name = tls_secret.model.metadata.name
    namespace = tls_secret.model.metadata.namespace
    gateway.cluster.do_action("delete", "secret", secret_name, "-n", namespace)
