...
def remove_preexisting_dns(dns_policy):
    """Remove DNS policy from conftest so we can create one via UI"""
    dns_policy.delete()
    yield
...
def test_dns_policy_ui(
    request, navigator, cluster, blame, gateway, dns_provider_secret, create_method, module_label, client
):
