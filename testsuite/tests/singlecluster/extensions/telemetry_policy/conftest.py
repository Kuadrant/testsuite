...
def service_monitor(cluster, request, testconfig, blame, limitador):
    ...
    match_labels = {"app": limitador.name()}
...
def limitador_metrics(prometheus, service_monitor, client, auth):
    ...
    """
    # auth_obj = auth[0]  # Use only auth_obj object from auth fixture
    client.get_many("/get", 5, auth=auth)
