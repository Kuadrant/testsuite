...
@pytest.fixture(scope="module")
def spicedb_hostname(exposer, gateway, blame):
    """Hostname for SpiceDB"""
    return exposer.expose_hostname(blame("spicedb-hostname"), gateway)


@pytest.fixture(scope="module")
def spicedb_route(request, gateway, spicedb, blame, spicedb_hostname, module_label, cluster):
    """
    Create an HTTPRoute to SpiceDB through the Gateway for external access from test suite.
    """
    route = HTTPRoute.create_instance(cluster, blame("spicedb"), gateway, {"app": module_label})
    route.add_hostname(spicedb_hostname.hostname)
    route.add_backend(spicedb)
    request.addfinalizer(route.delete)
    route.commit()

    spicedb.set_gateway_url(f"http://{spicedb_hostname.hostname}")
