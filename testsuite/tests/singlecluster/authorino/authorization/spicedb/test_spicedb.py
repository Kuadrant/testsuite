...
def create_secret(request, cluster, blame, spicedb_info, module_label, testconfig, kuadrant):
    """Create a secret for Authorino to use in authorization with SpiceDB."""
...
def authorization(authorization, create_secret, spicedb_info, schema_config):
    ...
    authorization.authorization.add_spicedb(
        ...
        spicedb_info["url"],
        create_secret,
        subject_type=schema_config.to_json("subject_type"),
        resource_type=schema_config.to_json("resource_type"),
        permission1=("GET", schema_config.to_json("permission1")),
        permission2=("POST", schema_config.to_json("permission2")),
...
def spicedb_query(spicedb, schema_config, relationship_config):
    ...
    """
    schema = spicedb.create_schema(schema_config)
    relationship = spicedb.create_relationship(relationship_config)
