...
def authorization(authorization, mockserver_expectation):
    ...
    authorization.rules.metadata.add_http("first", mockserver_expectation, "GET", priority=0)
    expectation_path_with_prev_uuid_param = mockserver_expectation + "?prev_uuid={auth.metadata.first.uuid}"
    authorization.rules.metadata.add_http("second", expectation_path_with_prev_uuid_param, "GET", priority=1)
