...
def pytest_collection_modifyitems(session, config, items):  # pylint: disable=unused-argument
    ...
    try:
        ...
        if project.connected:
            ...
            # Add Kuadrant version info
            for image, cluster_url in kuadrant_version():
                for item in items:
                    item.user_properties.append(("kuadrant_image", image))
                    item.user_properties.append(("kuadrant_cluster", cluster_url))
