...
class EnvoyWaitMixin:
    ...
    def set_metrics_route(self, metrics_route):
        ...
        if metrics_route is not None:
            from testsuite.tests.singlecluster.conftest import get_kuadrant_configs_value
    ...
    def wait_for_envoy_applied(self, timeout=120):
        ...
        from testsuite.tests.singlecluster.conftest import wait_for_policy_applied_to_envoy

        success = wait_for_policy_applied_to_envoy(
            self._metrics_route, self._initial_kuadrant_configs, timeout=timeout
        )
        assert success, (
