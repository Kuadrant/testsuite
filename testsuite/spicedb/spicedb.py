...
class SchemaConfig:
    ...
    def to_json(self, field: str = None) -> str:
        """Return full JSON string or a single field value as JSON."""
        data = asdict(self)
        if field:
            return data[field]
...
class SpiceDBService:
    ...
    @backoff.on_predicate(backoff.expo, lambda x: x is False, max_tries=5, jitter=backoff.full_jitter)
        ...
        check_request = CheckPermissionRequest(
            ...
            subject=SubjectReference(
                object=ObjectReference(
                    object_type=schema_config.subject_type, object_id=relationship_config.subject_ids[0]
                )
            ),
        )
