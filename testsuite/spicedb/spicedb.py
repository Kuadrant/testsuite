...
class SpiceDB(Backend):
    ...
    @property
    def reference(self):
        """Returns the backend reference for Gateway routing (uses HTTP port)"""
        return {
            "group": "",
            "kind": "Service",
            "port": 8443,
    ...
    def set_gateway_url(self, gateway_url: str):
        """
        Set the Gateway URL for accessing SpiceDB externally.
        """
    ...
    def http_url(self):
        """
        Returns the HTTP API URL for SpiceDB client (test suite access).
        """
    ...
    def grpc_url(self):
        ...
        """
        return f"{self.name}.{self.cluster.project}.svc.cluster.local:{self.grpc_port}"

    @property
    def client(self):
        """
        Lazy-initialized SpiceDB client for managing schemas and relationships.
        Uses HTTP API for external access (test suite via Gateway).
        """
