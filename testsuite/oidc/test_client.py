...
class ClientConfig:  # pylint: disable=too-many-instance-attributes
    ...
    client_id: str
    client_type: str  # "confidential", "public", "service"
...
class OIDCTestClient:
    ...
    def __init__(self, keycloak_oidc_client: KeycloakOpenID, client_id: str, client_secret: Optional[str] = None):
    ...
    def validate_token(self, token_string: str) -> dict:
        ...
        token = jwt_lib.decode(token_string, options={"verify_signature": False})

        # Basic validations
        assert "openid" in token["scope"], "Token should include OpenID scope"
        assert token["typ"] == "Bearer", "Should be a Bearer token"
        assert token["azp"] == self.client_id, f"Token should be issued for {self.client_id}"
    ...
    def create_confidential_client(cls, keycloak, hostname: str, client_id: str = "my-confidential-client"):
        ...
        return cls(oidc_client, client_id, keycloak_client.secret), keycloak_client
    ...
    def create_public_client(cls, keycloak, hostname: str, client_id: str = "my-public-client"):
        ...
        return cls(oidc_client, client_id, None), keycloak_client
