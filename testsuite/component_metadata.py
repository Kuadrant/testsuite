...
class ReportPortalMetadataCollector:
    ...
    def add_properties_to_items(self, items):
        ...
        for cluster_name in ["cluster1", "cluster2", "cluster3"]:
            if cluster_name not in self.all_cluster_metadata:
                continue

            cluster_data = self.all_cluster_metadata[cluster_name]
            if "kuadrant_image" not in cluster_data["metadata"]:
                continue

            property_value = self._build_property_value(cluster_data)
            for item in items:
                item.user_properties.append((cluster_data["console_url"], property_value))

    def _build_property_value(self, cluster_data):
